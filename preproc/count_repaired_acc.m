%count_repaired
%%This script count the number of repaired volumes in each run. written by
%%Jin Wang 5/20/2019. It will print out the name of the run (the first column) the number of volumes being
...
CCN.func_n='sub*Phon*'; %sub*Sem*
CCN.ses='ses-7';
n=6; %number of consecutive volumes being replaced. no more than 6 consecutive volumes being repaired.
writefile='accuracy_Phon_behavior_ses-7.txt'; %'accuracy_Sem_behavior_ses-7.txt';
bids_folder='/dors/booth/JBooth-Lab/BDL/ELP/Jin_code/bids';
subjects = {};
...
hdr='subjects run_name acq_date cond1 acc1 rt1 cond2 acc2 rt2 cond3 acc3 rt3 cond4 acc4 rt4';
fprintf(fid, '%s', hdr);
fprintf(fid, '\n');
for i=1:length(subjects)
    ...
    for j=1:length(func_f)
        ...
        for ii=1:size(conditions,1)
            ...
            acc_thiscond=data.calculated_accuracy((sum((data.trial_type==thiscondition)')==4)');
            rt_thiscond=data.calculated_RT(((sum((data.trial_type==thiscondition)')==4)'),:);
        ...
        fprintf(fid,'%s %s %s %s %.4f %.6f %s %.4f %.6f %s %.4f %.6f %s %.4f %.6f\n', ...
            subjects{i}, run_name, shifted_data_acq, conditions(1,:), acc_by_condition(1,:), rt_by_condition(1,:), ...
